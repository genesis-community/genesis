...
use Genesis::Top;
use Genesis::Env;
use_ok 'Genesis::Env::SecretsStore::Vault';
use Genesis;

fake_bosh;

# To select tests, specify `prove <args> <file> :: test_id_1 test_id_2 ...`

# Test ID: initialize
subtest 'initialize vault secret store' => sub {
    plan skip_all => 'initialize not selected test'
        if @ARGV && ! grep {$_ eq 'initialize'} @ARGV;
...
EOF
    ...
    my $ss;
    lives_ok {$ss = $env->secrets_store} "Can instantiate a secrets store for environment";
    ok ref($ss) eq "Genesis::Env::SecretsStore::Vault", "Instanciated secrets store is a Vault secrets store";

    # Default values
    ok $ss->default_mount eq '/secret/', "Vault secret store default mount is '/secret/'";
    ok $ss->mount eq '/secret/', "Vault secret store mount using the default mount";
    ok $ss->default_slug eq 'standalone/thing', "Vault secret store default slug is '<env>/<type>'";
    ok $ss->slug eq 'standalone/thing', "Vault secret store slug is using the default slug";
    ok $ss->base eq '/secret/standalone/thing/', "Vault secret store base is the mount + the slug + '/'";
    ok $ss->root_ca_path eq '', "Vault secret store has no root ca path";

    # `cp /Users/dbell/.replyrc \$HOME/` unless -f $ENV{HOME}."/.replyrc"; use Pry; pry;

    teardown_vault();
};

subtest 'override values for vault secret store' => sub {
	plan skip_all => 'override_values not selected test'
		if @ARGV && ! grep {$_ eq 'override_values'} @ARGV;
...
genesis:
  env: standalone
  min_version: 2.8.0
  root_ca_path: shhh/root_ca/cert
  secrets_mount: shhh
  secrets_path:  /super-cool-env/lab/
...
EOF
    ...
    # Default values
    ok $ss->default_mount eq '/secret/', "Vault secret store default mount is '/secret/'";
    ok $ss->mount eq '/shhh/', "Vault secret store mount using the formatted version of provided mount of '/shhh/'";
    ok $ss->default_slug eq 'standalone/thing', "Vault secret store default slug is '<env>/<type>'";
    ok $ss->slug eq 'super-cool-env/lab', "Vault secret store slug is using formatted version of provided path";
    ok $ss->base eq '/shhh/super-cool-env/lab/', "Vault secret store base is the provided mount + provided slug + '/'";
    ok $ss->root_ca_path eq '/shhh/root_ca/cert', "Vault secret store has a specified root ca path";
