...
is set, downloads deployment artifacts to the specified directory and exits.
When
C<--history> is set, appends the full deployment history to the output.  When
the kit defines an C<info> hook and neither C<$timestamp> nor C<--history> is
active, the hook is invoked after the standard output block.
...
=item C<"Cannot use timestamp argument with environments that do not have deployment audit logs.  Please ensure the environment file specifies genesis.minimum_version of at least 3.1.0, and that the manifest_store is set to exodus or hybrid in the <repo>/.genesis/config file.">
...
=item C<"The 'deployments' command is not yet fully implemented.">

Currently always raised when invoking this command.

=back

B<Returns:> Nothing.  Currently bails with a not-yet-implemented message.

B<Examples:>

  # Attempt to list deployment history for an environment:
  # CLI: genesis prod-us-east deployments
  # bails: The 'deployments' command is not yet fully implemented.
