package Genesis::Env::SecretsStore::Vault;
use strict;
use warnings;

use Genesis;
use Genesis::Vault;
use Genesis::UI (qw/checkbox in_controlling_terminal/);
...
sub new {
	...
	# validate call
	my @required_options = qw/connection/;
...
sub kit_secrets_parser {
	my ($self) = @_;

	unless (defined($self->{__secrets_parser})) {
		my $parser;
		my $parser_source = self->env->kit->secrets_source;
		if ($parser_source eq 'kit') {
			require Genesis::Kit::SecretsParser::FromKit;
			$parser = Genesis::Kit::SecretsParser::FromKit->new(
				$self,
				$self->env->dereferenced_kit_metadata,
				@{$self->env->features},
			)
		} elsif ($parser_source eq 'manifest') {
			require Genesis::Kit::SecretsParser::FromManifest;
			$parser = Genesis::Kit::SecretsParser::FromManifest->new(
				$self,
				# TODO: Need to generate and pass in manifest (without causing cycling)
			)
		} else {
			bug(
				"#R{[ERROR]} Unknown secrets source #C{parser_source} - expecting kit or manifest"
			)
		}
		$self->{__secrets_parser} = $parser;
	}
	return $self->{__secrets_parser};
}
...
sub generate {
	...
	_validate_option({
		required => [],
		optional => [qw/verbose filter/],
	}, %opts);

	my $processing_opts = {
		level=>$opts{verbose}?'full':'line'
	};
	my $ok = $self->_process_kit_secret_plans(
		'add',
		$self->env,
