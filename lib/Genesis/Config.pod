...
nested calls to skip loading without setting C<persistent_signature>.
...
C<persistent_signature> upon completion, after which C<loaded()> returns
true. See the non-existent file note under C<_load()> for cases where
C<persistent_signature> remains unset.
...
C<persistent_signature> is not set, so C<loaded()> returns false
...
is disabled, C<_load()> completes without setting C<persistent_signature>.
