...
=item * B<set> - values written programmatically via C<set()>
...
=head2 new
    ...
    my $cfg = Genesis::Config->new($path, $autosave);
...
C<"Cannot set a key in the configuration without a key"> — raised if C<$key>
is undefined.
...
B<Returns:> Nothing. This method is called for its side effects only.
...
If differences exist, prints a formatted diff showing what changed. If no
differences are found, raises a C<bug()> assertion since this condition should
not occur (the caller should not invoke C<show_diff()> when nothing changed).
...
B<Examples:>
    ...
    $cfg->show_diff;
    # prints a spruce-format diff of changes, or calls bug() if no diff
...
Triggers lazy loading from disk if needed.
...
Modifies the object in-place (C<loaded_values>, C<persistent_signature>).
...
B<Examples:>
    ...
    $self->_load;
    # Loads $self->{path} into loaded_values; sets persistent_signature on success
...
For C<array> and C<boolean> types, normalizes the value in-place: booleans
are coerced to JSON::PP boolean objects; array values parsed from environment
strings are split and element-wise converted.
