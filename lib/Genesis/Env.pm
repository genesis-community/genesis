...
sub iaas {
	...
	) if $self->use_create_env && !$iaas && $fatal;
...
sub get_environment_variables {
  ...
  $env{GENESIS_MIN_VERSION_FOR_KIT}      = $self->kit->genesis_version_min();
	if ($self->exists) {
		$env{GENESIS_ENV_IAAS}               = $self->iaas($hook =~ /(cloud-config|manifest|deploy)/);
		$env{GENESIS_ENV_SCALE}              = $self->scale($hook =~ /(cloud-config|manifest|deploy)/);
...
sub deploy {
	...
	if ($manifest_store ne 'repository') {
		...
		my $exodus_deployment = {
			...
			manifest_type => $pruned_deploy_manifest->type,
			deployer      => $exodus->{deployer},
			kit_id        => $self->kit->id,
			kit_is_dev    => $exodus->{kit_is_dev},
