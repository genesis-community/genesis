...
sub _validate_value {
	...
	unless ($self->get('is_ca')) {
		...
		@sans{grep {@names || $_ ne $subjectCN} @SANs}=();
		@desired_sans{@names}=();
		my @extra_sans = sort(grep {!exists $desired_sans{$_}} keys %sans);
		my @missing_sans = sort(grep {!exists $sans{$_}} keys %desired_sans);
...
sub _get_safe_command_for_add {
	...
	@names = (sprintf("ca.n%09d.%s", rand(1000000000),$self->get('base_path')))
		if $self->get('is_ca') && ! scalar(@names);
...
sub _get_safe_command_for_rotate {
	...
	@names = $self->read_names if $self->get('is_ca') && ! scalar(@names);
...
sub _base_safe_command {
	...
	if ($self->get('is_ca')) {
		push(@cmd, '--subject', "cn=".$names[0]);
		push(@cmd, '--name', $names[0]); # Temporary hack untill safe supports SANs-less CAs
	} else {
