...
use Genesis qw/uniq run bug debug compare_arrays/;
...
sub _get_safe_command_for_rotate {
	...
	my $regen_keys  = delete $opts{regen_x509_keys};
	my $update_subj = delete $opts{update_subject};
	debug("X509 _get_safe_command_for_rotate: unconsumed opts: %s", join(', ', sort keys %opts)) if %opts;
	...
	if ($action eq 'issue') {
		...
		push(@cmd, '--subject', "cn=$cert_name")
			if $update_subj || envset("GENESIS_RENEW_SUBJECT");
	}
	return @cmd
}
