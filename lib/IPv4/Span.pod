...
=head2 add
  ...
  my $new_span = $span->add($other_span);

Adds one span to another, or increases the size of the span by the given number.  If the number is negative, it increases the size of the span by growing the span from before the start address. 
  ...
  # Add a positive number to the span
  my $span = IPv4::Span->new('192.168.1.1', '192.168.1.5');
  my $new_span = $span->add(5);
  print "New span after adding 5: ", $new_span->range(), "\n"; # prints '192.168.1.1-192.168.1.10'

  # Add a negative number to the span
  $new_span = $span->add(-2);
  print "New span after adding -2: ", $new_span->range(), "\n"; # prints '192.168.0.255-192.168.1.10'

=head2 subtract

  my $new_span = $span->subtract($other_span);

Subtracts one span from another, or decreases the size of the span by the given number.  If the number is negative, it decreases the size of the span by shrinking the span by increasing the starting address.
